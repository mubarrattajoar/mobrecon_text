...
class DenseStack_Backnone(nn.Module):
    def __init__(self, input_channel=128, out_channel=24, latent_size=256, kpts_num=21, pretrain=True):
        ...
        #added by mub
        self.text_dim = 256
        self.txt_fusion = conv_layer(
            1024 + self.text_dim,  # in_channels
            1024,  # keep it at 1024
            ks=1,
            stride=1,
            padding=0,
            bias=False,
    ...
    def forward(self, x, text_token):
        ...
        B, C, H, W = stack2_mid.shape  # C == 1024
        txt_map = text_token.view(B, self.text_dim, 1, 1) \
            .expand(-1, -1, H, W)  # (B,256,H,W)
        merged = torch.cat([stack2_mid, txt_map], dim=1)  # (B,1280,H,W)
        fused = self.txt_fusion(merged)  # (B,1024,H,W)
        latent = self.mid_proj(fused)
